          ...
          : (
            <>
              ...
              <div className='flex justify-end mt-8'>
                <button onClick={() => navigate('/checkout')} disabled={cartState.data.every(item => !item.selected)} className={`${cartState.data.every(item => !item.selected) ? 'bg-gray-300 hover:bg-gray-300 cursor-not-allowed' : 'bg-black hover:bg-gray-700 cursor-pointer'} text-white rounded-md px-7 py-2 transition`}>Checkout</button>
              </div>
            </>
          )
        }
